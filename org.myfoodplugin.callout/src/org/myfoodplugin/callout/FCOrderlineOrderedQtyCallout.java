...
public class FCOrderlineOrderedQtyCallout implements IColumnCallout {
	...
	public String start(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value, Object oldValue) {
		...
		Integer customizedID = (Integer) mTab.getValue("GM_Customized_ID");
		if (customizedID == null || customizedID <= 0) {
			return null;
		}
		BigDecimal priceCustom = BigDecimal.ZERO;
			...
			try (PreparedStatement pstmt = DB.prepareStatement(sql, null)) {
				pstmt.setInt(1, customizedID); // 將foodMenuID塞到？
		...
		BigDecimal subAmount = priceActual.multiply((BigDecimal) value);
